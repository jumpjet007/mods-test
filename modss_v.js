...
(function () {
	...
	var TRASH_R = ['$$$####!!!!!!!', '^^^^^^##@', '@!^^!@#@@$$$$$', '^^#@@!!@#!$', '@#!@@@##$$@@'];
	var version_modss = '3.0', API = 'https://api.lampa.stream/', type = '', jackets = {}, cards, ping_auth, manifest, menu_list = [], vip = false, leftVipD = '', user_id = 86914, uid = '3cd26a8132842c8ee74bfe73d0ff3bbb_86914', IP, logged = true;
	...
	var Modss = {
      ...
      if(Lampa.Storage.get('showModssVip', false) && leftVipD && logged) setTimeout(function (){
        Modss.showModssVip();
    	...
    	if (/*Lampa.Storage.get('pro_pub') && */Lampa.Storage.get('pub_tv')) 
    ...
    sources: function () {
      ...
      Lampa.Modal.render().addClass('modal--cub-premium').find('.modal__content').before('<div class="modal__icon"><svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 32 32"><path d="m2.837 20.977q-.912-5.931-1.825-11.862a.99.99 0 0 1 1.572-.942l5.686 4.264a1.358 1.358 0 0 0 1.945-.333l4.734-7.104a1.263 1.263 0 0 1 2.1 0l4.734 7.1a1.358 1.358 0 0 0 1.945.333l5.686-4.264a.99.99 0 0 1 1.572.942q-.913 5.931-1.825 11.862z" fill="#D8C39A"></svg></div>');
		...
		online: function (back) {
		  var params = {
      	url: '',
      	title: Lampa.Lang.translate('modss_title_online') + " MODS's " + leftVipD,
      ...
      Lampa.Notice.listener.follow('select',function (e) {
		...
		balansers: function() {
		  var balansers = {"hdr":"HDR MODS's [4K]","pub":"Pub 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","hdrezka":"HDRezka 4K","bazon":"Bazon 4K","qiwi":"VBMovies 4K ","uakino":"UAKino","eneida":"Eneida(Ukr)","kodik":"Kodik","anilibria":"Anilibria","hdvb":"HDVB  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","kinobase":"Kinobase","cdnmovies":"CDNmovies","videocdn":"VideoCDN","rezka":"Rezka","collaps":"Collaps","kinotochka":"KinoTochka"};
...
function rezka(component, _object) {
    ...
    var extract = {};
    var prox = Protocol() +'prox.lampa.stream/';//'https://cors.nb557.workers.dev/' + (IP ? 'ip' + IP + '/' : '');
  ...
  function cdnmovies(component, _object) {
    ...
    var proxy = 'https://cors.nb557.workers.dev/' + (IP ? 'ip' + IP + '/' : '');
    var embed = 'https://skinny-wilderness.cdnmovies-stream.online/';
    var iframe_proxy = true;
  ...
  function component(object) {
    ...
    this.requestParams = function (url) {
      ...
      query.push('cub_id=aGFyaW1hbWJ1cmFAZ21haWwuY29t');
      query.push('uid=3cd26a8132842c8ee74bfe73d0ff3bbb_86914');
      if (cards) {
				var notices = Lampa.Storage.get('account_notice', []).filter(function (n) {
    ...
    this.similars = function (json) {
      ...
      json.forEach(function (elem) {
        ...
        if(orig && orig !== name) info.push(orig);
  			if (elem.type) info.push(elem.type == 'tv-series' || elem.type == 'serial' || elem.type == 'MINI_SERIES' ? 'Cериал': 
      ...
      this.get = function (){
        if (object.url.indexOf('pub/tvs') > -1) {
          ...
          network['native'](object.url, function (str) {
    	    ...
    	    var pl = [{"name":"Kulik","url":"kulik"},{"name":"FreeTv","url":"free"}]
          _this1.list(pl);
    		}
      };
      this.parsePlaylist = function (data) {
        var postdata = data ? {datas: data, id:'aGFyaW1hbWJ1cmFAZ21haWwuY29t'} : {url: object.url, id:'aGFyaW1hbWJ1cmFAZ21haWwuY29t'};
    ...
    var servOpt = 'Выбрать сервер';
  	...
  	this.build = function (data, f) {
          ...
          if (Lampa.Storage.field('mods_tv_style') !== 'cats' && object.url.indexOf('pub/tv') == -1) html.append(info.append(filter.render()));
    		...
    		if (object.url.indexOf('Fork') == -1 && Lampa.Storage.field('mods_tv_style') == 'vert') {
    	  	html.find('.scroll:eq(1)').css({'position':'relative', 'min-height':'30em'}).append('<div class="epg" style="position:absolute;width:100%;top:15em;left:0;"><div class="program"></div></div>');
  	...
  	this.append_v = function (data) {
  		...
  		data.forEach(function (element) {
  			...
  			if ((object.url.indexOf('Fork') > -1 || Lampa.Storage.field('mods_tv_style') == 'cats') || object.url.indexOf('pub/tv') > -1) 
            ...
            card.find('.card__img').remove();
  			...
  			card.on('hover:focus hover:touch', function (e, is_mouse) {
  				if (!element.title && !element.results && (object.url.indexOf('pub/tv') == -1 && (Lampa.Storage.field('mods_tv_style') == 'cats' && object.url.indexOf('Fork') == -1)) && (Lampa.Storage.field('mods_tv_style') == 'cats' || Lampa.Storage.field('mods_tv_style') == 'vert'))
  ...
  function Radio_n(object) {
	...
	function startPlugin() {
      ...
      Lampa.SettingsApi.addParam({
				...
				field: {
					name: '🆔 86914 🔹  💻 Windows NT 10.0 (x64)'
				}
			});
			Lampa.SettingsApi.addParam({
				component: 'settings_modss',
